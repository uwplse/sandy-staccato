...
def read_OF_data(folder):
    ...
    for i in files:
        ...
        totals[k] = [total]
    f = [ os.path.join(folder, f) for f in os.listdir(folder) if os.path.isfile(os.path.join(folder,f)) and \
          os.path.basename(f).startswith("error_") ]
    err_count = 0
    for i in f:
        err_count += read_OF_error(i)
    return (plots,totals, err_count)
        ...
        while i < n_ev:
            ...
            while i < n_ev and \
                ...
                if ev[7] == "false":
                    ...
                    break
            if (i - start_i) > 5:
                print "wildly expected result", (i - start_i)
            if is_bad:
                continue
            e_series = None
...
#    event_series_avg = {}
    ...
    return (event_series,event_points, errors)
...
def read_subsonic_data(file_name):
    ...
    for thread_id, thread_ev in time_series.iteritems():
        ...
        while i < n_ev:
            ...
            if i == n_ev:
                print >> sys.stderr, "MISSING NEXT FOR (2)", ev
                break
            ...
            # we don't have the next event in the sequence. weird
                ...
                print >> sys.stderr, "MISSING NEXT FOR", ev, thread_ev[i]
                #print thread_ev[i]
                continue
